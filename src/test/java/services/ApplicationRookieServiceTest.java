...
@Transactional
public class ApplicationRookieServiceTest extends AbstractTest {
	...
	public void driverListApplication() {
		...
		Object testingData[][] = {
				...
				{ "", "rookie1", null },
				// Positive test: Listing accepted applications of the rookie
				{ "ACCEPTED", "rookie1", null },
				// Positive test: Listing pending applications of the rookie
				{ "PENDING", "rookie1", null },
				// Positive test: Listing rejected applications of the rookie
				{ "REJECTED", "rookie1", null },
				// Positive test: Listing submitted applications of the rookie
				{ "SUBMITTED", "rookie1", null },
	...
	private void templateListApplication(String status, String username, Class<?> expected) {
		...
		try {
			...
			List<Application> allApplications = rookie.getApplications();
			List<Application> filterApplications = (List<Application>) this.applicationService
					.getApplicationsByRookieAndStatus(rookie, res);

			if (status.contentEquals(""))
				Assert.isTrue(allApplications.size() == rookie.getApplications().size());
			else
	...
	public void driverCreateApplication() {
		...
		Object testingData[][] = {
				// Positive test: Create an application as a rookie
				{ "rookie1", null },
	...
	public void driverUpdateApplication() {
		...
		Object testingData[][] = {
				// Positive test: Update an application as a rookie
				{ a, "http://www.solution.com", "explication1", "rookie1", null },
				// Negative test: Trying to update an application with an invalid URL
				{ a, "aaaa", "explication2", "rookie1", ConstraintViolationException.class },
