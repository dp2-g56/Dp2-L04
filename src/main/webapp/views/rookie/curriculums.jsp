...
<security:authorize access="hasRole('ROOKIE')">
	...
	<display:table pagesize="5" name="curriculums" id="row" class="displaytag" 
					requestURI="/curriculum/rookie/list.do">
			
		<display:column property="title" titleKey="curriculum.title" /> 
		
		<display:column titleKey="curriculum.action">
			<spring:url var="currUrl" value="/curriculum/rookie/show.do">
				<spring:param name="curriculumId" value="${row.id}"/>
			</spring:url>
			<spring:url var="currEditUrl" value="/curriculum/rookie/edit.do">
				<spring:param name="curriculumId" value="${row.id}"/>
			</spring:url>
			<spring:url var="currDeleteUrl" value="/curriculum/rookie/delete.do">
				<spring:param name="curriculumId" value="${row.id}"/>
			</spring:url>
	...
	<spring:url var="newCurrUrl" value="/curriculum/rookie/new.do"/>
	<p><a href="${newCurrUrl}">
