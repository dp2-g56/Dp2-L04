...
public interface FinderRepository extends JpaRepository<Finder, Integer> {
	...
	public List<Rookie> getRookiesThatFinderKeyWordIsContaine(String rs, String rt, String title, String desription, String requiredProfile, String ticker);

	@Query("select p from Position p where p.deadline = ?1 and p.isDraftMode = false and p.isCancelled = false")
	public List<Position> getPositionsByDeadline(Date deadLine);

	@Query("select p from Position p where (p.deadline < ?1 or p.deadline = ?1) and p.isDraftMode = false and p.isCancelled = false")
	public List<Position> getPositionsByMaxDeadline(Date maxDeadLine);

	@Query("select p from Position p where (p.offeredSalary > ?1 or p.offeredSalary = ?1) and p.isDraftMode = false and p.isCancelled = false")
